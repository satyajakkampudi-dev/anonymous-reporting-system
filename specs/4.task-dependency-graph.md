...
### Task MP-FIX-ADMIN-LOAD-LIMIT - admin reads cap silently at 20 (only the oldest 20 reports ever load)

**Type:** Fix (post-implementation - touches the anonymity read gateway)
**Dependencies:** LIB-FOUNDATION (gateway `loadReportsForAdmin`), A-F2 (dashboard aggregation), A-D-queue / nav-queue, A-F18 (SLA digest), MP-FIX-ADMIN-NAV-DISPLAY-ROUTING
**Points:** 0.5
**Tag:** `shared-lib`, `anonymous-admin`, `fix`, `pagination`, `anonymity-gateway`

**Description:**
The single admin read gateway `loadReportsForAdmin` (LIB-FOUNDATION, `lib/access.js`) only forwards a `limit` to `loadCollectionWithQuery` when the caller passes one (`if (limit !== undefined) opts.limit = limit;`). Every live consumer calls it as `loadReportsForAdmin({})` with NO limit and NO sort - `anonymous-admin/src/frames/nav-dashboard.js` (A-F2 stats), `nav-queue.js` (A-F4/A-F5 queue, then in-memory paginate), `app-start.js`, and `sla-digest.js`. With no limit, `loadCollectionWithQuery` falls back to its framework default of `limit = 20` (`@frontmltd/frontmjs/core/Collection.js`), and with no sort MongoDB returns rows in natural/insertion order - i.e. the **oldest 20**. Once the `reports` collection exceeds 20 rows, every newly-created report (inserted after the 20th) is never read back into ANY admin screen: it is invisible in the queue and dashboard for BOTH primary and secondary, because the cap sits UPSTREAM of the role filter (`roleSees`), and it is undercounted in the A-F2 dashboard aggregates. The in-memory `byNewestFirst` sort in `lib/queue.js` reorders only the 20 that were loaded, which masks the cap - the list *looks* newest-first while silently missing everything past the 20th oldest row. (Note: `MP-FIX-QUEUE-PAGINATION` specified server-side paging that would have set an explicit `limit`/`skip`, but the current `nav-queue.js` does in-memory slicing over `loadReportsForAdmin({})` - that task is not reflected in the live code, so the cap is active.) This fix makes the gateway default to a generous full-working-set limit AND a `createdOn`-descending sort when the caller supplies neither, in ONE place, so all four consumers are corrected at once and the existing in-memory filter/recusal/priority/pagination design is preserved unchanged.

**Files to modify** (surgical patch):

- `lib/constants.js` - add `ADMIN_FULL_LOAD_LIMIT` (the full-working-set cap, e.g. `1000`) with a comment explaining it is the gateway's default when no explicit `limit` is passed, and why a high cap + sort is needed (the queue/dashboard filter, recuse, priority-float and paginate in memory).
- `lib/access.js` - in `loadReportsForAdmin`, when `limit` is `undefined`, set `opts.limit = ADMIN_FULL_LOAD_LIMIT`; when `sort` is `undefined`, set `opts.sort = { createdOn: -1 }`. An explicit `limit`/`sort` from a caller (e.g. `loadReportForAdmin` passing `limit: 1`, or the export path) must still win. Keep `projection: adminProjection` and the `applyAdminProjection(extractRowData(r))` mapping exactly as-is.

**Files to create:** none.

**Acceptance criteria:**

- [ ] With > 20 reports in the collection, a freshly-submitted report appears in BOTH the primary and secondary admin queue (subject only to the existing role filter / recusal) and is counted in the A-F2 dashboard aggregates.
- [ ] `loadReportsForAdmin({})` returns the full working set (up to `ADMIN_FULL_LOAD_LIMIT`), newest-first by `createdOn`.
- [ ] An explicit `limit` or `sort` passed by a caller (e.g. `loadReportForAdmin` `{ limit: 1 }`, `export-report.js`) is still honoured - the defaults apply ONLY when the caller omits them.
- [ ] `adminProjection` is still applied (identity-free output) and `extractRowData` extraction is unchanged - no identity field leaks.
- [ ] `npm run build` succeeds in both apps that bundle `lib/`.
- [ ] No change to `lib/queue.js`, the queue renderer, or the in-memory pagination - the cap is fixed solely in the gateway.

**Claude Code prompt:**

```
Read .claude/CLAUDE.md § "Invocation Lifecycle - CRITICAL" and § "Field Access Patterns" (extractRowData / gateway context).
Read lib/access.js (loadReportsForAdmin / loadReportForAdmin / extractRowData / applyAdminProjection),
lib/constants.js (LIST_PAGE_SIZE region), and the four consumers
anonymous-admin/src/frames/{nav-dashboard,nav-queue,app-start,sla-digest}.js (read only - do NOT change them).

Invoke /frontm-api-verify on loadCollectionWithQuery's { query, projection, sort, limit } options
(confirm the default limit is 20 and that sort/limit are accepted). Emit the Verification Report.
Do NOT proceed if any API is NOT FOUND - ask the user instead.

Apply a SURGICAL PATCH:
1. lib/constants.js - add `export const ADMIN_FULL_LOAD_LIMIT = 1000;` near LIST_PAGE_SIZE with a
   comment: the gateway's default limit when no explicit limit is passed; the admin queue/dashboard
   filter, recuse, priority-float and paginate IN MEMORY, so the gateway must load the full working
   set (loadCollectionWithQuery defaults to only 20 → oldest-20 → new reports invisible).
2. lib/access.js - in loadReportsForAdmin, import ADMIN_FULL_LOAD_LIMIT, and after building
   `const opts = { query, projection: adminProjection };`:
     if (limit !== undefined) opts.limit = limit; else opts.limit = ADMIN_FULL_LOAD_LIMIT;
     if (sort !== undefined)  opts.sort  = sort;  else opts.sort  = { createdOn: -1 };
   Leave the loadCollectionWithQuery call, projection, rows mapping and applyAdminProjection untouched.

Do NOT:
- Change any of the four consumer frames, lib/queue.js, or the queue renderer.
- Alter adminProjection / ADMIN_EXCLUDED_FIELDS / extractRowData.
- Invent any frontm.ai API.
...
### Task MP-FIX-QUEUE-SERVER-PAGINATION - admin queue fetches per page from the DB (supersedes the in-memory slice + the load-all ceiling)

**Type:** Fix (post-implementation - feature; touches the anonymity read gateway + the shared report schema)
**Dependencies:** MP-FIX-ADMIN-LOAD-LIMIT, MP-FIX-QUEUE-PAGINATION (superseded), LIB-FOUNDATION, A-F2, A-D-queue / nav-queue, A-F4 (role + recusal), A-F5 (priority), U-F8 (submit transforms)
**Points:** 1.5
**Tag:** `shared-lib`, `anonymous-admin`, `anonymous-user`, `fix`, `pagination`, `anonymity-gateway`

**Description:**
After MP-FIX-ADMIN-LOAD-LIMIT the admin queue loaded the FULL report set (up to ADMIN_FULL_LOAD_LIMIT) and paginated it IN MEMORY - correct but unbounded: it re-fetches every row on every page click and has a hard ceiling the maritime-compliance volume can plausibly reach. MP-FIX-QUEUE-PAGINATION specified server-side paging but was never reflected in the live code, and its plan could not float priority server-side because the framework has NO aggregation (loadCollectionWithQuery takes only a simple sort). This task makes the queue genuinely server-paginated - one bounded DB read per page click - by introducing a STORED priority sort key. A new hidden `priorityRank` column (0 = priority, 1 = normal, dbName `priorityRank`) is written on EVERY report save via `reportDoc.onSave` in BOTH apps (the universal save chokepoint, since adminReportDoc IS the shared reportDoc) from the SHARED `priorityRankFor` predicate (`isPriority ? 0 : 1`), so it can never drift from the dashboard's `isPriority`. The queue query keys role on `assignedTo` (PRIMARY → PRIMARY-routed; SECONDARY → all), narrows by quick-filter (status, or `{ priorityRank: 0 }` for the Priority chip), and sorts `{ priorityRank: 1, createdOn: -1 }` - priority floats, newest-first within band. Free-text recusal (`accusedParty`) is not a query, so it trims the fetched page in-code (a page may render < PAGE_SIZE in the rare recusal case, D9). The gateway gains a `skip` passthrough; nav-queue over-fetches by 1 (`limit: PAGE_SIZE + 1`) for `hasMore`. Legacy rows predating the column are stamped ONCE by `backfillPriorityRank` (two targeted `updateMany` calls - constant cost at any volume - writing ONLY the derived field, no Doc.save → no version/modifiedOn/audit churn, NFR-3), latched cross-admin in a sharedField and run from admin app-start. The dashboard (A-F2) + SLA digest keep their full-set loads (they must see everything); the ADMIN_FULL_LOAD_LIMIT ceiling from MP-FIX-ADMIN-LOAD-LIMIT stays for them. The queue renderer is unchanged - it already consumes the QUEUE_REPORTS / QUEUE_PAGE / QUEUE_HAS_MORE stash and renders renderPaginationControls.

**Files modified:**
- `lib/dashboard-stats.js` - add `priorityRankFor` (stored 0/1 key) + `priorityQueryCriteria` (the isPriority OR in query form, single-sourced for the backfill).
- `lib/queue.js` - replace the in-memory producer (`buildQueueReports`/`roleSees`/`sortForQueue`/`applyQuickFilter`) with `buildQueueQuery` ({query, sort} from role + quick-filter + priorityRank float) + `projectQueueRows` (in-code recusal + identity-free projection of a page). Keep `isRecusedFor`, `isPriority` re-export.
- `lib/access.js` - `loadReportsForAdmin` gains a `skip` passthrough; add `backfillPriorityRank` (two `updateMany`, audit-free).
- `anonymous-user/src/sections/report-details.js` + `anonymous-admin/src/sections/manual-log.js` - add hidden NUMBER_FIELD `priorityRankField` (dbName `priorityRank`).
- `anonymous-user/src/frames/report-validation.js` - extend `reportDoc.onSave` to stamp priorityRank.
- `anonymous-admin/src/frames/priority-rank-writer.js` (NEW) - bind `adminReportDoc.onSave` to stamp priorityRank; registered in `anonymous-admin/src/main.js`.
- `anonymous-admin/src/frames/nav-queue.js` - server-side per-page fetch (query+sort, skip, limit PAGE_SIZE+1, in-code recusal).
- `anonymous-admin/src/frames/app-start.js` - run-once `backfillPriorityRank` behind a sharedField latch.
- `anonymous-admin/src/constants.js` - `SHARED_KEYS.PRIORITY_RANK_BACKFILL_DONE` + TTL.

**Files to create:** `anonymous-admin/src/frames/priority-rank-writer.js` (done).

**Acceptance criteria:**
- [ ] Each page click (prev/next) and each quick-filter chip issues ONE bounded DB read (limit PAGE_SIZE+1, skip = page*PAGE_SIZE) - the queue never loads the full set.
- [ ] Priority reports (CRITICAL severity / IMMEDIATE urgency / ESCALATED) float to the top of the All view across pages; newest-first within each band.
- [ ] Role visibility preserved: PRIMARY never sees SECONDARY-routed (ESCALATED/against-admin) reports; recusal still hides an against-this-admin report (in-code page filter).
- [ ] A newly-submitted report appears immediately (page 1, correct band) for the assigned role; an escalate/override-severity re-floats it on next render (priorityRank re-stamped by onSave).
- [ ] Legacy rows are backfilled once with no audit/version churn; the latch prevents re-runs; a backfill failure retries next session and never blocks the shell.
- [ ] adminProjection still applied (identity-free); dashboard stats (A-F2) + SLA digest unchanged (separate full loads).
- [ ] `npm run build` succeeds in both apps.

**Claude Code prompt:** (implemented in this session - see Files modified; this entry is the execution-log record, append-only.)

---

### Task MP-FIX-DASHBOARD-ALERTS-ROLE-SCOPE - dashboard counts + alerts must be role-filtered (primary ≠ secondary)

**Type:** Fix (post-implementation)
**Dependencies:** A-F2 (dashboard), A-F19 (alerts), A-F4 (role filter), LIB-FOUNDATION, MP-FIX-QUEUE-SERVER-PAGINATION
**Points:** 0.5
**Tag:** `anonymous-admin`, `shared-lib`, `fix`, `anonymity`, `role-filter`

**Description:**
The dashboard aggregation (A-F2, nav-dashboard + app-start) and the in-app Alerts breach list (A-F19) computed over the UNFILTERED `loadReportsForAdmin({})` set, so primary and secondary admins saw identical counts - and a PRIMARY admin saw aggregates of ESCALATED / against-admin reports it cannot open, a partial D9 anonymity leak. REQUIREMENTS §3 / the A-F4 sequence require dashboard + queue to be role-filtered; only the queue was. Fix: a shared in-memory `roleVisibleReports(reports, viewingRole, identity)` (lib/queue.js) reusing the routing chokepoint (resolveTargetRoleFor) + the recusal predicate; the dashboard producers filter before `buildDashboardStats` and stash the role-visible set (STATE_KEYS.ALERTS_REPORTS) for the sync Alerts renderer. The SLA-digest JOB stays unfiltered (auto-escalation must consider all reports); only the per-admin UI is scoped.

**Files modified:** `lib/queue.js` (roleVisibleReports), `anonymous-admin/src/frames/nav-dashboard.js`, `anonymous-admin/src/frames/app-start.js`, `anonymous-admin/src/sections/display/alerts/index.js`, `anonymous-admin/src/constants.js` (STATE_KEYS.ALERTS_REPORTS).

**Acceptance criteria:**
- [ ] Primary dashboard counts cover only PRIMARY-routed reports; secondary covers all. The two roles no longer show identical numbers when escalated/against-admin reports exist.
- [ ] Alerts breach list is role-scoped (reads the stash; falls back to full set if absent).
- [ ] SLA-digest job auto-escalation still scans all reports (unchanged).
- [ ] adminProjection still applied; `npm run build` succeeds.

**Claude Code prompt:** (implemented in this session - execution-log record, append-only.)

---

### Task MP-FIX-CONTACT-OPEN-REPORTING - reporter may opt to be identifiable; admin sees voluntarily-shared contact

**Type:** Fix (post-implementation - deliberately relaxes NFR-1 for the open-reporting path)
**Dependencies:** U-F7 (contact), A-D-managecontent, U-F5 (anonymity preview), LIB-FOUNDATION, C1 (anonymity gateway)
**Points:** 1
**Tag:** `anonymous-user`, `anonymous-admin`, `shared-lib`, `fix`, `privacy-model`

**Description:**
Product change: the "How we can reach you" section is OPEN reporting - a reporter who provides contact is choosing to be identifiable, and the compliance team should see those details in the admin report view. Previously `contactMethod`/`contactValue` were in ADMIN_EXCLUDED_FIELDS (never shown). Fix: remove them from ADMIN_EXCLUDED_FIELDS (the U-F5 "what the admin will see" preview auto-updates via isAdminVisibleField - no drift); add an informed-consent `info` hint on the submit form's contact field; bind contactMethod/contactValue in the admin bundle (manual-log section, hidden on the create form, contactValue encrypted:true so the read lambda decrypts); render a "Reporter contact (shared voluntarily)" block in the admin manage-content view (web + mobile) when a value is present. The reporter's PLATFORM identity (reporterId + reporter-create audit createdBy/modifiedBy) STAYS excluded - the admin sees only what the reporter typed, never their userId.

**Files modified:** `lib/access.js` (ADMIN_EXCLUDED_FIELDS), `anonymous-user/src/sections/contact.js` (hint + header), `anonymous-admin/src/sections/manual-log.js` (bind contact fields), `anonymous-admin/src/sections/display/manage-content/{index,web,mobile}.js` (render).

**Acceptance criteria:**
- [ ] A report submitted WITH contact shows the method + details in the admin manage view; a report WITHOUT contact shows no contact block (anonymous).
- [ ] The submit form shows the informed-consent hint; the U-F5 preview lists contact under "what the admin will see".
- [ ] reporterId is still NEVER shown to admins.
- [ ] ⚠️ VERIFY ON LIVE: contactValue (encrypted: true) decrypts correctly admin-side. If it renders as ciphertext/garbage, the encryption key is user-scoped → drop `encrypted` on both sides (acceptable: voluntarily-shared open contact) and re-test on fresh data.
- [ ] `npm run build` succeeds in both apps.
...
### Task MP-FIX-DASHBOARD-GLOBAL-COUNTS - dashboard + alerts are a GLOBAL overview (supersedes MP-FIX-DASHBOARD-ALERTS-ROLE-SCOPE)

**Type:** Fix (post-implementation - product decision)
**Dependencies:** MP-FIX-DASHBOARD-ALERTS-ROLE-SCOPE (superseded), A-F2, A-F19
**Points:** 0.25
**Tag:** `anonymous-admin`, `shared-lib`, `fix`, `dashboard`

**Description:**
Product clarified: the dashboard is a shared ORG-WIDE overview - total reports, the escalated count, and every breakdown must be the SAME for both primary and secondary admins. Only the QUEUE stays role-restricted (what each role can act on). This SUPERSEDES MP-FIX-DASHBOARD-ALERTS-ROLE-SCOPE (which had role-scoped the dashboard + alerts): nav-dashboard + app-start now aggregate `buildDashboardStats` over the FULL gateway set and stash the full set for the alerts breach renderer; the in-memory `roleVisibleReports` helper is removed (now unused). REQUIREMENTS §3's "dashboard role-filtered" note applies to the QUEUE only going forward. Accepted tradeoff: a primary admin's escalated/total counts include against-admin reports routed to secondary (aggregate-only; details still gated by the queue role filter + recusal - no reportId/contact leak).

**Files modified:** `anonymous-admin/src/frames/nav-dashboard.js`, `anonymous-admin/src/frames/app-start.js`, `lib/queue.js` (removed roleVisibleReports). Alerts renderer + STATE_KEYS.ALERTS_REPORTS retained (now carries the full set).

**Acceptance criteria:**
- [ ] Total reports + escalated count + breakdowns are identical for primary and secondary.
- [ ] Queue remains role-scoped (primary ≠ secondary in the queue).
- [ ] `npm run build` succeeds.
...
### Task MP-FIX-SUBMIT-FRESH-FORM - Submit Report opens a fresh form (warm-Lambda carryover overwrote the previous report)

**Type:** Fix (post-implementation)
**Dependencies:** U-F4 (openSubmitReport), U-F8 (submit transforms - the `if (!reportId)` idempotency mint)
**Points:** 0.25
**Tag:** `anonymous-user`, `fix`, `buffer`

**Description:**
`openSubmitReport` (nav-submit-report.js) deferred the fresh-draft reset ("a later task" per its header comment) and rendered `reportDoc` as-is after `Context.CreateAndInit`. On a WARM Lambda the module singletons still hold the previously submitted report - docId, reportId, every content field and the evidence attachments - so the form opened pre-filled, and because U-F8 mints `reportId` only when ABSENT (double-submit idempotency), a second submission reused the old id and `save()` silently OVERWROTE the previous report row in MongoDB. Cold starts mask the bug entirely. `resetEvidenceSlots()` resets slot VISIBILITY only, not values, so it does not mitigate. This fix transplants the admin manual-log opener's in-place reset (rule 26 ordering: new docId FIRST, then clear all field values) into `openSubmitReport`.

**Files to modify:**

- `anonymous-user/src/frames/nav-submit-report.js`

**Acceptance criteria:**

- [ ] `openSubmitReport.onResolution` sets `reportDoc.docId = state.getUniqueId()` and nulls every `reportDoc.fields` value BEFORE `sendSubmitGuard()` / `resetEvidenceSlots()` / `reportDoc.sendResponse()`.
- [ ] The stale "fresh-draft reset is a later task" header comment is replaced.
- [ ] No other handler in the file changes; the U-F5 guard and slot reset still run in their original order after the reset.
- [ ] `npm run build` succeeds in anonymous-user.
- [ ] Smoke test (warm container): submit report A → reopen Submit Report → form is BLANK; submit report B → My Reports lists BOTH A and B with distinct tracking ids.

**Claude Code prompt:**

```
Read .claude/CLAUDE.md § "App Entry-Point Bootstrap - CRITICAL" (rule 26 in-place
reset ordering). Read anonymous-admin/src/frames/nav-manual-log.js steps 1-2 (the
canonical reset). Apply the same reset to openSubmitReport.onResolution in
anonymous-user/src/frames/nav-submit-report.js immediately after
Context.CreateAndInit: new docId first, then `for (const field of
reportDoc.fields) field.value = null;`. Update the header comment that defers the
reset. Run npm run build.
```

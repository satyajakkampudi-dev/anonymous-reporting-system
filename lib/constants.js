...
export const ADMIN_ROLE = {
  PRIMARY: "quitelineprimaryadmin",
  SECONDARY: "quitelinesecondaryadmin",
};
...
// Per-USER-scoped stable tab contextId (framework-mapping rule 37). A stable per-screen
// string keeps ONE user's clicks in ONE tab (no per-click proliferation), but the
// framework broadcasts a rendered response to EVERY client on the same contextId STRING
// (the same mechanism that mirrors a user's web + mobile sessions). So a GLOBALLY
// identical id leaks one admin's UI into another admin's tab. Namespacing by userId keeps
// the id stable per user yet unique ACROSS users: same-user multi-device still mirrors
// (same userId), different users never share a channel. Falls back to conversationId then
// a constant so it never yields "undefined" (which would re-collide).
export const userTab = (contextId, s) =>
  `${contextId}_${s?.user?.userId || s?.conversationId || "anon"}`;
...
export const MSG = {
  ...
  // user -> admin: a participant JOINED and the call-queue was claimed join-driven (the
  // user/owner app's joinMeeting handler claims when an admin joins - needed for MOBILE,
  // where lifting CallKit doesn't run the web Answer A-F21). Carries { meetingId, attendedBy,
  // callRef }; the admin receiver sets that admin busy. Mirrors healthMariner joinMeetingIntent.
  CALL_CLAIMED: "MSG_CALL_CLAIMED",
  // user -> admin: the meeting ended / a participant left (the user app, as the meeting
  // OWNER, is where the Loft backend fires the endMeeting/leaveUser lifecycle intents - see
  // anonymous-user/.../contracts/call-lifecycle.js). Carries { meetingId, attendedBy }; the
  // admin receiver frees the answering admin's presence (busy -> available). Mirrors the
  // healthMariner notifyMedicalMember -> medicalMeetingEnded hop.
  CALL_ENDED: "MSG_CALL_ENDED",
  // Intra-admin-bot notification (framework-mapping rule 32). Carries escalate /
  // auto-escalate / manual-log notifications to the assigned admins' userIds so the
  // adminNotifyReceiver runs in EACH recipient's own session and pushes-to-self
  // (mobile+web). Payload { reportId, event } - identity-free.
  ADMIN_NOTIFY: "MSG_ADMIN_NOTIFY",
};
...
// ⚠️ SCHEDULER TEST MODE - shrinks the auto-escalate / auto-close / SLA-digest delays
// from days to MINUTES so the scheduled jobs can be exercised in a live test session.
// SET BACK TO false BEFORE PRODUCTION (it restores the real D2/D11 delays below).
// This is the ONLY switch; the prod values are kept inline so revert is a one-flag flip.
const SCHEDULER_TEST_MODE = true;

export const TIMING = {
  // CRITICAL +1 day (D2) - TEST: 2 min.
  AUTO_ESCALATE_CRITICAL_MS: SCHEDULER_TEST_MODE
    ? 2 * ONE_MIN_MS
    : 1 * ONE_DAY_MS,
  // others +3 days (D2) - TEST: 3 min.
  AUTO_ESCALATE_DEFAULT_MS: SCHEDULER_TEST_MODE
    ? 3 * ONE_MIN_MS
    : 3 * ONE_DAY_MS,
  // resolved +30 days (D2) - TEST: 3 min.
  AUTO_CLOSE_MS: SCHEDULER_TEST_MODE ? 3 * ONE_MIN_MS : 30 * ONE_DAY_MS,
  CALL_RING_TIMEOUT_MS: 30 * 1000, // 30 s ring before voicemail (D7) - unchanged
  ...
  CALL_INACTIVITY_TIMEOUT_MS: 2 * 60 * 60 * 1000, // 2 hours - unchanged
  // OPEN / ESCALATED 24 h backstop digest (D11) - TEST: 2 min.
  SLA_OPEN_MS: SCHEDULER_TEST_MODE ? 2 * ONE_MIN_MS : 1 * ONE_DAY_MS,
  SLA_ESCALATED_MS: SCHEDULER_TEST_MODE ? 2 * ONE_MIN_MS : 1 * ONE_DAY_MS,
  // SLA backstop digest sweep cadence (A-F18) - daily prod; TEST: 2 min.
  SLA_DIGEST_INTERVAL_MS: SCHEDULER_TEST_MODE ? 2 * ONE_MIN_MS : 1 * ONE_DAY_MS,
};
...
export const STATIC_DATA_KEYS = {
  ...
  // DOMAIN-scoped file bucket. A FILE_FIELD with fileScope:"domain" lands here (NOT in
  // conversationsBucket, which is where IMAGE_FIELD / conversation-scoped media go) at
  // `${currentUserDomain}/${key}` - readable by any user in the domain (anonymity-safe:
  // no conversationId in the path). Mirrors healthMarinerCommonLib (crewApi.js uses
  // "ContentS3Bucket" for its domain-scoped FILE_FIELD documents). Per FrontM platform
  // team (Jaswanth, 2026-06-10). NOTE (ops): confirm this static-data key is provisioned
  // for the onship domain - if getStaticData returns null, evidence cannot be signed.
  CONTENT_BUCKET: "ContentS3Bucket",
  ...
  ADMIN_BOT_ID: "anonymousadminbotid",
  USER_BOT_ID: "anonymoususerbotid",
  // Masked/system host email for the anonymous voice call (lib/calling.maskedHostEmail).
  // A REAL FrontM user (a dedicated system "call host" - never the reporter/admin; D8).
  ANON_CALL_HOST_EMAIL: "anonymousemail",
  // The FrontM "Loft" meeting-player HOST (bare host, NO protocol, NO path) that the web
  // client opens the call in: https://<lofthost>/<roomId>. This is OUR app's own key (NOT
  // SeaMedix's medical "telemedUrl" - same Loft infra, but we don't depend on the medical
  // app's key). Devops sets it per env (stage/prod); on dev it's unset and the code falls
  // back to CALLING.LOFT_HOST (dailydev.frontm.ai). videoCall.serverUrl MUST be this BARE
  // host - feeding the full Daily room URL makes the client build a broken
  // "https://https//frontm.daily.co/room/room" URL (double protocol + duplicated room).
  MEETING_LOFT_HOST: "anonCallLoftHost",
};
...
// Rows per page for the custom-HTML list views (framework-mapping rule 36):
// admin Report Queue + user My Reports (server-side skip/limit) and admin Alerts
// (in-memory slice). Shared so all lists page uniformly.
export const LIST_PAGE_SIZE = 10;

// The admin read gateway's default `limit` when a caller passes none
// (lib/access.js loadReportsForAdmin). `loadCollectionWithQuery` defaults to
// only `limit = 20` with no sort, so a bare `loadReportsForAdmin({})` would
// silently load just the OLDEST 20 reports by insertion order - every report
// past the 20th becomes invisible in BOTH admin roles (the cap sits upstream of
// the role filter) and is undercounted on the dashboard. The admin queue +
// dashboard filter, recuse, priority-float and paginate IN MEMORY over the full
// set, so the gateway must load the whole working set (MP-FIX-ADMIN-LOAD-LIMIT).
export const ADMIN_FULL_LOAD_LIMIT = 1000;
...
export const CALLING = {
  ...
  // Fallback FrontM "Loft" meeting-player host for onship DEV (bare host, no protocol/path).
  // Confirmed from the JWT redirectionUrl the framework mints for onship. The web client
  // opens the call at https://<this>/<roomId>. Used as videoCall.serverUrl when the
  // STATIC_DATA_KEYS.MEETING_LOFT_HOST ("telemedUrl") static data is not set. The
  // stage/prod hosts differ (SeaMedix stage = dailystage.frontm.ai) → prefer static data.
  LOFT_HOST: "dailydev.frontm.ai",
  // Native incoming-call ringtone for the VoIP/CallKit push (SeaMedix SOUND_NAME).
  RING_SOUND: "incallmanager_ringback.mp3",
};

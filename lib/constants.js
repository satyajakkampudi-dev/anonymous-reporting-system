...
// FrontM platform role codes surfaced in state.user.roles (granted via the bot's
// userRoles / license keys — see deployment.config userRoles). These gate WHICH APP
// a user may OPEN — distinct from the app-internal PRIMARY/SECONDARY routing role
// (ROLE / ADMIN_ROLE above), which comes from the admin-users registry. The admin app
// opens for APP_ROLES.ADMIN; the registry then supplies PRIMARY/SECONDARY (defaulting
// to PRIMARY for a FrontM-admin not yet curated in the registry). Mirrors the
// sailors-cart admin access gate (sailors-admin/src/frames/access-gate.js).

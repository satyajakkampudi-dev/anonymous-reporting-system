...
// host (no protocol, no path). Prefer OUR deployment static data (STATIC_DATA_KEYS
// .MEETING_LOFT_HOST = "anonCallLoftHost"); fall back to the onship-dev host
// (CALLING.LOFT_HOST = dailydev.frontm.ai). Best-effort + logged.
...
export const ringAvailableAdmins = async ({
  ...
  // SILENT bot-to-bot trigger only — fires each admin's X3 receiver, which delivers BOTH the
  // web ring (RING_START) AND the MOBILE VoIP/CallKit push (ringVoipSelf). The VoIP push is
  // sent from the admin's OWN session (X3) — NOT fanned out from here — because it must carry
  // the ADMIN's freshly-minted meeting TOKEN so a lifted CallKit call can actually join
  // (healthMariner incommingQueueEntryIntent pattern). A reporter-side fan-out can't mint
  // per-admin tokens and lands on web as a stray toast — hence this is trigger-only.
...
// MOBILE ring (VoIP/CallKit), called by the X3 receiver in the rung admin's OWN session.
// Uses sendVoipCallPushNotifications to the admin's own CONVERSATION (device-token push →
// reaches mobile CallKit, does NOT toast web). The payload carries the ADMIN's meeting
// TOKEN (videoSessionId) + Loft host (mediasoupHost) so lifting the CallKit call joins the
// Daily room directly — the healthMariner incommingQueueEntryIntent pattern (it sets
// pushData.videoSessionId = videoCall.meetingToken then sendVoipCallPushNotifications).
// Caller mints the token first (adminVideoCall.getAccessToken) and passes it here.
// Identity-free (ER-A5): generic message, no caller datum. Best-effort + logged.
export const ringVoipSelf = async ({
  meetingId,
  meetingToken,
  loftHost,
  videoControlId,
  ...
  try {
    ...
    if (!conversation) {
      D.log({
        message: "ringVoipSelf — no conversation; skip VoIP push",
        data: { meetingId },
      });
      return { ok: false };
    }
    // Payload SHAPE mirrors the proven healthMariner generatePushData (queueRouting.js) so
    // the mobile CallKit client does a LOFT-TRACKED join (which fires the joinMeeting
    // lifecycle on the owner bot — the signal our join-driven claim hooks). Earlier our push
    // carried meetingType:"anonymous-call" (an invented value) and omitted videoControlId/
    // tabid; the mobile client appears to take a non-tracked direct-media path for anything
    // other than meetingType:"regular" + a bound videoControlId, so NO joinMeeting fired and
    // the call was never claimed. Match the reference exactly for the join-critical fields.
    const data = {
      videoSessionId: meetingToken, // the admin's jwt — lets the lifted CallKit call join
      meetingId,
      mediasoupHost: loftHost, // Loft/meeting server host
      callAction: "CallStart", // tells the native client this is an incoming call
      video: false, // voice-only
      iAmHost: true, // reference sets true for the joining participant's push
      startWithVideoMuted: false,
      preConnectCallCheck: true,
      botId: state.botId,
      userDomain: state.currentUserDomain,
      userId: state.user?.userId,
      meetingType: "regular", // MUST be "regular" — the tracked-join path (was "anonymous-call")
      videoControlId, // binds the join to the admin's VideoCall instance (was missing)
      tabid: state.getUniqueId(), // per-call tab id the mobile client opens (was missing)
      roomName: CALLING.RING_MESSAGE,
      title: CALLING.RING_MESSAGE,
      soundName: CALLING.RING_SOUND,
    };
    await state.notification.sendVoipCallPushNotifications(
      [conversation],
...
// Resolve a SINGLE admin by their email — regardless of current availability.
// Needed by the user-app join-driven claim: the "joinMeeting" lifecycle event carries
// the joiner's EMAIL (state.messageFromUser.userEmail — the healthMariner joinMeetingIntent
// keys off the same field), NOT a reliable userId. Our call-queue / notify path is
// userId-keyed, so we map joiner-email → adminUserId via the seeded registry. Unlike
// resolveAvailableAdmins this does NOT filter on availability — by the time the admin's
// join fires they may already be flipping to busy. Returns { adminUserId, adminEmail } or
// null (joiner is not a known admin → the reporter or a stray participant → no claim).
export const resolveAdminByEmail = async (adminEmail) => {
  ...
  try {
    ...
    D.log({
      message: "resolveAdminByEmail — admin-users query failed",
      data: { adminEmail, error: String(error) },
    });
    return null;
  }
  const rows = adminUsersCollection.rows || [];
  for (const r of rows) {
    const data = extractRowData(r);
    if (data && data.adminUserId) {
      return { adminUserId: data.adminUserId, adminEmail: data.adminEmail };
    }
  }
  D.log({
    message: "resolveAdminByEmail — no admin row matched",
    data: { adminEmail },
  });
  return null;
};

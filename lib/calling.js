...
export const ringAvailableAdmins = async ({
  ...
  // Channel 1 — in-app trigger: fires each admin's X3 receiver (web ring banner + RING_START).
  ...
  // Channel 2 — VoIP/CallKit push per admin (this is what reaches MOBILE — confirmed working
  // when present). It is fanned out from the reporter because the X3 receiver runs
  // server-side where state.client is always "web", so it cannot platform-gate the VoIP push.
  // Mobile renders a native CallKit incoming call; web has no CallKit so it may surface this
  // as a notification (the in-app banner + RING_START are the web ring) — accepted trade-off
  // to keep mobile ringing. Identity-free (ER-A5): generic message, no caller datum.
  // Best-effort + logged per admin; a push fault must never block the call.
  for (const userId of userIds) {
    try {
      await state.notification.sendVoipPushNotificationToUser(
        state.user?.userId, // caller context — not surfaced to the admin
        userId,
      ...
      D.log({
        message: "ringAvailableAdmins — VoIP push failed (non-fatal)",
        data: { userId, callRef, error: String(error) },
      });
    }
  }
  return { rung: userIds };

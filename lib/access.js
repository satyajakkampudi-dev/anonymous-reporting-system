...
// OPEN-REPORTING (MP-FIX-CONTACT-OPEN-REPORTING): contactMethod + contactValue are NO LONGER
// excluded - the reporter optionally chooses to be identifiable and those fields are shown in
// the admin report view (the submit form's `info` hint is the informed-consent disclosure, and
// the U-F5 "what the admin will see" preview auto-includes them via isAdminVisibleField below).
// The reporter's PLATFORM identity stays anonymous: reporterId + the reporter-create audit
// (createdBy/modifiedBy) remain excluded, so the admin sees ONLY what the reporter typed, never
// their userId. This is the deliberate relaxation of NFR-1 for the open-reporting path.
export const ADMIN_EXCLUDED_FIELDS = ["reporterId", "createdBy", "modifiedBy"];
...
export const loadReportsForAdmin = async ({
  query = {},
  limit,
  sort,
  skip,
  ...
  // Default to the full working set, newest-first, when the caller omits limit/sort.
  // loadCollectionWithQuery otherwise caps at 20 with no sort → only the oldest 20
  // rows load and every newer report is silently invisible (MP-FIX-ADMIN-LOAD-LIMIT).
  // An explicit caller limit/sort (e.g. loadReportForAdmin { limit: 1 }, or the queue's
  // per-page { limit: PAGE_SIZE+1, skip }) still wins (MP-FIX-QUEUE-SERVER-PAGINATION).
  opts.limit = limit !== undefined ? limit : ADMIN_FULL_LOAD_LIMIT;
  opts.sort = sort !== undefined ? sort : { createdOn: -1 };
  // skip is the server-side pagination offset (page * pageSize). Omitted → 0 (first page),
  // the framework default. The queue passes it per page click; the full-set callers don't.
...
// One-time backfill of the stored priorityRank sort key on LEGACY rows that predate the
// column (MP-FIX-QUEUE-SERVER-PAGINATION). New/edited reports get priorityRank from
// reportDoc.onSave; existing rows have none, and a MISSING priorityRank sorts as null -
// ABOVE 0 in the queue's ascending sort - so un-migrated rows would falsely float to the
// TOP. This stamps them with TWO targeted updateMany calls (constant cost, any volume):
//   1. priorityRank = 0 for rows missing it whose severity/urgency/status make them priority
//      (the SAME criteria as isPriority, via the single-sourced priorityQueryCriteria).
//   2. priorityRank = 1 for whatever rows are still missing it (the non-priority remainder).
// Order matters: call 1 first, then call 2's { $exists: false } only matches the rest.
// Writes ONLY the derived priorityRank (no Doc.save, so no version/modifiedOn/audit churn -
// the audit trail of real reports stays clean, NFR-3). Idempotent: once stamped, $exists is
// true and both calls match nothing. Best-effort + logged; a failure must never block the
// admin shell - the run-once latch is set by the caller only on success.
export const backfillPriorityRank = async () => {
  const collection = reportsCollection.name; // reports_${systemId}
  const operateOnParentDomain = reportsCollection.operateOnParentDomain;
  // 1. Legacy priority rows → 0.
  const priorityResult = await state.db.updateManyDataInCollection({
    collection,
    query: { priorityRank: { $exists: false }, ...priorityQueryCriteria() },
    document: { priorityRank: 0 },
    updateOperator: "$set",
    operateOnParentDomain,
  });
  // 2. Remaining legacy rows (non-priority) → 1.
  const normalResult = await state.db.updateManyDataInCollection({
    collection,
    query: { priorityRank: { $exists: false } },
    document: { priorityRank: 1 },
    updateOperator: "$set",
    operateOnParentDomain,
  });
  D.log({
    message: "backfillPriorityRank: stamped legacy rows",
    data: {
      priorityStatus: priorityResult?.statusCode,
      normalStatus: normalResult?.statusCode,
    },
  });
};

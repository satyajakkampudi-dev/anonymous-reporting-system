...
// SECONDARY_ADMIN does everything PRIMARY does, PLUS handles ESCALATED reports.
...
export const STATUS_TRANSITIONS = {
  [STATUS.OPEN]: [
    { to: STATUS.UNDER_REVIEW, actor: TRANSITION_ACTOR.ADMIN, action: ACTION.TAKE_REVIEW },
    { to: STATUS.ESCALATED, actor: TRANSITION_ACTOR.SYSTEM, action: "autoEscalate" },
    { to: STATUS.WITHDRAWN, actor: TRANSITION_ACTOR.REPORTER, action: ACTION.WITHDRAW },
  ],
  [STATUS.UNDER_REVIEW]: [
    { to: STATUS.RESOLVED, actor: TRANSITION_ACTOR.ADMIN, action: ACTION.RESOLVE },
    { to: STATUS.ESCALATED, actor: TRANSITION_ACTOR.ADMIN, action: ACTION.ESCALATE },
    { to: STATUS.ESCALATED, actor: TRANSITION_ACTOR.SYSTEM, action: "autoEscalate" },
    { to: STATUS.WITHDRAWN, actor: TRANSITION_ACTOR.REPORTER, action: ACTION.WITHDRAW },
  ],
  [STATUS.ESCALATED]: [
    { to: STATUS.UNDER_REVIEW, actor: TRANSITION_ACTOR.SECONDARY_ADMIN, action: ACTION.TAKE_REVIEW },
    { to: STATUS.RESOLVED, actor: TRANSITION_ACTOR.SECONDARY_ADMIN, action: ACTION.RESOLVE },
  ],
  [STATUS.RESOLVED]: [
    { to: STATUS.CLOSED_BY_USER, actor: TRANSITION_ACTOR.REPORTER, action: ACTION.ACCEPT },
    { to: STATUS.CLOSED_BY_SYSTEM, actor: TRANSITION_ACTOR.SYSTEM, action: "autoClose" },
    { to: STATUS.REOPENED, actor: TRANSITION_ACTOR.REPORTER, action: ACTION.REJECT },
  ],
  [STATUS.REOPENED]: [
    { to: STATUS.UNDER_REVIEW, actor: TRANSITION_ACTOR.ADMIN, action: ACTION.TAKE_REVIEW },
    { to: STATUS.ESCALATED, actor: TRANSITION_ACTOR.ADMIN, action: ACTION.ESCALATE },
    { to: STATUS.CLOSED_REJECTED, actor: TRANSITION_ACTOR.ADMIN, action: ACTION.CLOSE_REJECTED },
  ],
...
export const canTransition = (from, to, actor) => {
  ...
  return moves.some(
    (m) => m.to === to && actorSatisfies(m.actor, actor)
  );
};

...
// Secondary sort applied AFTER the priority float: NEWEST-first (largest createdOn first)
// so the most recently reported item surfaces at the top of each band. A missing createdOn
// sorts as 0 (oldest) so it falls to the bottom rather than the top.
const byNewestFirst = (a, b) =>
  (Number(b.createdOn) || 0) - (Number(a.createdOn) || 0);

// PRIORITY SORT (A-F5). Priority reports float to the TOP; within each band, newest-first.
